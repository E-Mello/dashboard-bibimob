...
const ProfileDropdown = () => {
  ...
  return (
    <div className="relative text-end flex rounded-lg " ref={dropdownRef}>
      ...
      {isOpen && (
        <div
          className={`flex relative self-end transition-opacity ${
            isOpen ? "opacity-100" : "opacity-0"
          }  duration-300 ease-in-out delay-75`}
        >
          <div
            className={`flex flex-col text-small text-black absolute right-0 mt-2 py-2 w-36 bg-[#685baa] rounded shadow-lg p-2 decoration gap-1 transition-opacity duration-300 ${
              isOpen ? "opacity-100" : "opacity-0"
            }`}
          >
            <button
